...
def uninstall(ctx, graph):
    ...
    for node_instance in ctx.model.node_instance.iter():
        ...
        tasks.append(WorkflowTask(uninstall_node_instance, node_instance=node_instance))
    graph.add_tasks(tasks)
    create_node_instance_task_dependencies(graph, node_instances, tasks, reverse=True)
